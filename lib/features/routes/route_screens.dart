        ...
        children: [
          if (!showResults) ...[
            _PlannerForm(
              ...
              onClearFrom: fromStation == null ? null : () => setState(() { fromStation = null; _routeOptions = const []; }),
              onClearTo: toStation == null ? null : () => setState(() { toStation = null; _routeOptions = const []; }),
            ...
            Text('${fromStation!.name} → ${toStation!.name}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
            const SizedBox(height: 6),
            Text('${_formatDeparture(departureTime)} • $selectedTransport', style: Theme.of(context).textTheme.bodySmall),
            ...
            if (recommended == null)
              ...
              RecommendationSelector(
                ...
                onChanged: (value) {
                  if (value == 'Cheapest') {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Cheapest recommendation will activate when verified fare data is connected.')),
                    );
                    return;
                  }
              ...
              if (_routeOptions.length > 1) ...[
                ...
                Text('${_routeOptions.length} route options checked', style: Theme.of(context).textTheme.bodySmall),
              ],
              const SizedBox(height: 18),
          ...
          selected == 'Fastest'
              ...
              : selected == 'Least Walking'
                  ...
                  : selected == 'Fewest Transfers'
                      ...
                      : 'Verified fare data is required before cheapest-route ranking can be used.',
...
class _PlannerForm extends StatelessWidget {
  const _PlannerForm({required this.fromStation, required this.toStation, required this.departureTime, required this.selectedTransport, required this.profileTransport, required this.transportChangedForJourney, required this.onPickFrom, required this.onPickTo, required this.onClearFrom, required this.onClearTo, required this.onSwap, required this.onPickDepartureTime, required this.onTransportChanged});
  ...
  Widget build(BuildContext context) {
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _StationField(label: 'From', hint: 'Select starting station', station: fromStation, icon: Icons.trip_origin_rounded, onTap: onPickFrom, onClear: onClearFrom),
          Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Align(alignment: Alignment.centerRight, child: IconButton(tooltip: 'Swap stations', onPressed: onSwap, icon: const Icon(Icons.swap_vert_rounded)))),
          _StationField(label: 'To', hint: 'Select destination', station: toStation, icon: Icons.location_on_rounded, onTap: onPickTo, onClear: onClearTo),
          const SizedBox(height: 16),
          InkWell(
            borderRadius: BorderRadius.circular(12),
            onTap: onPickDepartureTime,
            child: InputDecorator(
              decoration: const InputDecoration(labelText: 'Departure Time', prefixIcon: Icon(Icons.schedule_rounded), suffixIcon: Icon(Icons.edit_calendar_rounded)),
              child: Text(_formatDeparture(departureTime), style: const TextStyle(fontWeight: FontWeight.w700)),
            ),
          ),
          const SizedBox(height: 18),
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: ['All', 'MRT', 'LRT', 'Bus', 'KTM'].map((transport) => ChoiceChip(label: Text(transport), selected: selectedTransport == transport, onSelected: (_) => onTransportChanged(transport))).toList(),
          ),
          if (profileTransport != 'All') ...[
            const SizedBox(height: 10),
            Text(transportChangedForJourney ? 'Profile preference: $profileTransport • This journey: $selectedTransport' : 'Using preferred transport from Profile: $profileTransport', style: Theme.of(context).textTheme.bodySmall),
          ],
        ],
      ),
    );
...
class _StationField extends StatelessWidget {
  const _StationField({required this.label, required this.hint, required this.station, required this.icon, required this.onTap, required this.onClear});
  final String label;
  final String hint;
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: onTap,
      child: InputDecorator(
        decoration: InputDecoration(labelText: label, prefixIcon: Icon(icon), suffixIcon: station == null ? const Icon(Icons.keyboard_arrow_down_rounded) : IconButton(onPressed: onClear, icon: const Icon(Icons.close_rounded))),
        child: Text(station?.name ?? hint, style: TextStyle(color: station == null ? Theme.of(context).hintColor : null, fontWeight: station == null ? FontWeight.normal : FontWeight.w700)),
      ),
    );
  }
}
...
class RouteSearchResultCard extends StatelessWidget {
  const RouteSearchResultCard({required this.result, required this.recommendation, super.key});
  final RouteSearchResult result;
  final String recommendation;

  @override
  Widget build(BuildContext context) {
    final transferText = result.transfers == 0 ? 'Direct journey' : result.transfers == 1 ? '1 transfer' : '${result.transfers} transfers';
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            ...
            Expanded(child: Text('$recommendation recommendation', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900))),
            StatusChip(transferText, color: AppColors.primary),
          ]),
          const SizedBox(height: 14),
          Text(result.modes.isEmpty ? 'Walking connection' : result.modes.join(' + '), style: const TextStyle(fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          Text('${result.stops} stops • ${result.walkingConnections} walking connection${result.walkingConnections == 1 ? '' : 's'}'),
          const SizedBox(height: 8),
          const Text('Official timetable timing and verified fare will replace route-structure ranking when those data sources are connected.'),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return AppCard(
      child: Column(children: [
        ...
        const Text('No route found', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
        const SizedBox(height: 8),
        Text(transport == 'Bus' ? 'Bus route data is not connected yet. Try All, MRT, LRT, or KTM.' : 'Try another transport type or choose different stations.', textAlign: TextAlign.center),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 14),
        Text(ready ? 'Not calculated yet' : 'Select your journey first', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
        const SizedBox(height: 8),
        Text(ready ? '${fromStation!.name} to ${toStation!.name}' : 'Choose a starting station and destination.'),
        if (ready) ...[const SizedBox(height: 8), const Text('Only verified fare data will be displayed here.')],
      ]),
    );
  }
}
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        for (var i = 0; i < legs.length; i++)
          ListTile(
            ...
            leading: CircleAvatar(child: Icon(legs[i].mode == 'Walk' ? Icons.directions_walk_rounded : legs[i].mode == 'Bus' ? Icons.directions_bus_rounded : Icons.train_rounded, size: 20)),
            title: Text(legs[i].line, style: const TextStyle(fontWeight: FontWeight.w800)),
            subtitle: Text('${legs[i].from} → ${legs[i].to}'),
          ),
        const SizedBox(height: 6),
        Text('Schedule timing will be added when the official route data source is connected.', style: Theme.of(context).textTheme.bodySmall),
      ]),
    );
  }
}
...
class _RidershipInsightsCardState extends State<RidershipInsightsCard> {
  ...
  void _refresh() => setState(() => _snapshotFuture = _ridershipService.fetchLatestSnapshot());
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<RidershipSnapshot>(
      future: _snapshotFuture,
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return const AppCard(child: SizedBox(height: 150, child: Center(child: CircularProgressIndicator())));
        ...
        return AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [const Expanded(child: SectionTitle('Ridership Insights')), IconButton(onPressed: _refresh, icon: const Icon(Icons.refresh_rounded))]),
          const SizedBox(height: 12),
